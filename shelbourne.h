 ...
 *   - Mutes amplifier
 *   - Powers off amplifier
 *   - Blanks display
 *   - Turns off LED
 *
 * WiFi (DM870 module) stays on — use shelbourne_wifi_set() to power
 * it down for deeper sleep.
 *
 ...
 *
 * Returns immediately — the amplifier may produce distorted output
 * for the first few seconds while it stabilizes.
 *
...
/*
 * Control the DM870 WiFi/Bluetooth module power (GPIO 14, active low).
 *
 * on=1: power on the module (default state after init).
 * on=0: power off the module to save power during deep sleep.
 *
 * After powering off and back on, the WiFi connection takes ~10 seconds
 * to re-establish. The keypad remains functional regardless.
 */
...
/*
 * Keypad event: which key, and whether it was pressed or released.
 *
 * The driver delivers separate press and release events for every
 * button. This lets you detect:
 *
 *   Short press:  press event, then release within some threshold.
 *   Long press:   press event, hold (no release), act after a timeout.
 *   Repeat:       not generated by hardware — implement in your timer
 *                 loop while a key is held.
 *
 * Long-press example (2-second hold on POWER):
 *
 ...
 *
 * Reads all pending press and release events from the keypad driver.
 * Returns the number of events written to the array (0 if none pending).
 *
 ...
 *
 * The fd is opened O_NONBLOCK. Use it in select()/poll() to sleep
 * efficiently until a button is pressed — avoiding CPU-wasting spin
 * loops (e.g., during standby waiting for the power button).
 *
 * When select() signals readability, call shelbourne_keypad_poll() to
 * retrieve the decoded events. Do not read() from the fd directly —
 * the raw bytes need decoding.
 *
